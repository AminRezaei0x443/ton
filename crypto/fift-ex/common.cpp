...
td::Result<vm::StackEntry> json_to_stack_entry(td::JsonObject &obj) {
  ...
  if (type == "string") {
    TRY_RESULT(data, get_json_object_string_field(obj, "value"));
    return vm::StackEntry(data, false);
  }
  if (type == "bytes") {
    TRY_RESULT(data, get_json_object_string_field(obj, "value"));
    TRY_RESULT(data_bytes, td::base64_decode(data));
    return vm::StackEntry(data_bytes, true);
  }
  if (type == "builder") {
    TRY_RESULT(data, get_json_object_string_field(obj, "value"));
    TRY_RESULT(data_bytes, td::base64_decode(data));
    TRY_RESULT(boc, vm::std_boc_deserialize(data_bytes));
    TRY_RESULT(data_cell, boc->load_cell());
    auto b = td::make_ref<vm::CellBuilder>();
    b.write().append_cellslice(td::make_ref<vm::CellSlice>(vm::CellSlice(data_cell)));
    return vm::StackEntry(b);
  }
  if (type == "null") {
...
td::Result<std::string> stack_entry_to_json(vm::StackEntry se) {
  ...
  if (se.type() == vm::StackEntry::Type::t_string) {
    auto v = se.as_string();
    return R"({ "type": "string", "value": ")" + v + R"("})";
  }
  if (se.type() == vm::StackEntry::Type::t_bytes) {
    auto v = se.as_bytes();
    auto bf = td::BufferSlice(v);
    auto value = td::base64_encode(bf);
    return R"({ "type": "bytes", "value": ")" + value + R"("})";
  }
  if (se.type() == vm::StackEntry::Type::t_builder) {
    auto& b = *se.as_builder();
    TRY_RESULT(boc, vm::std_boc_serialize(b.finalize_copy()));
    auto value = td::base64_encode(boc);
    return R"({ "type": "builder", "value": ")" + value + R"("})";
  }
  if (se.is_null()) {
    return R"({ "type": "null" })";
  }
